...
/** LANGGUAGE:START **/
define("LANG_CATEGORIAS","Categoriass");
define("LANG_REMEDIOS","Remedioss");
define("LANG_ID","Ids");
define("LANG_TITULO","Titulos");
define("LANG_IMAGEM","Imagems");
define("LANG_DESCRICAO","Descricaos");
define("LANG_TEXTO","Textos");
...
if($_SESSION["IS_ADMIN"]==true){
	...
	$tags_html .= "</ul>" ;
	$tags_html .= "<h3 class=\"text-muted\">Remedios Naturais</h3>" ;
	$tags_html .= "<p>Cura pela natureza</p>" ;
	$tags_html .= "</div>" ;
	...
	if($_GET["table"]=="categoriass"){
		$tags_html .= "<li class=\"active\"><a href='?table=categoriass'>". @LANG_CATEGORIAS."</a></li>" ;
	}else{
		$tags_html .= "<li><a href='?table=categoriass'>". @LANG_CATEGORIAS."</a></li>" ;
	}
	if($_GET["table"]=="remedioss"){
		$tags_html .= "<li class=\"active\"><a href='?table=remedioss'>". @LANG_REMEDIOS."</a></li>" ;
	}else{
		$tags_html .= "<li><a href='?table=remedioss'>". @LANG_REMEDIOS."</a></li>" ;
	}
	...
	switch($_GET["table"]){
		case "home":
					$tags_html .= "<h4 class=\"page-title\">Remedios Naturais</h4>" ;
					$tags_html .= "<div class=\"row\">" ;
					$tags_html .= "<div class=\"col-md-4\">" ;
					$tags_html .= "<div class=\"panel panel-primary\" >" ;
					$tags_html .= "<div class=\"panel-heading\">" ;
					$tags_html .= "<h4  class=\"panel-title\"><span class=\"glyphicon glyphicon-list-alt\"></span> ". @LANG_CATEGORIAS ."</h4>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "<div class=\"panel-body\" style=\"min-height: 150px;\">" ;
					$tags_html .= "<ul class=\"list\">" ;
					/** fetch data from mysql **/
					$sql_query = "SELECT * FROM `categorias` ORDER BY `id`  DESC LIMIT 0 , 5" ;
					...
					$tags_html .= "</ul>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "<div class=\"panel-footer text-right\"><a href=\"?table=categoriass\" class=\"btn btn-sm btn-primary\">".LANG_MORE."</a></div>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "<div class=\"col-md-4\">" ;
					$tags_html .= "<div class=\"panel panel-success\" >" ;
					$tags_html .= "<div class=\"panel-heading\">" ;
					$tags_html .= "<h4  class=\"panel-title\"><span class=\"glyphicon glyphicon-list-alt\"></span> ". @LANG_REMEDIOS ."</h4>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "<div class=\"panel-body\" style=\"min-height: 150px;\">" ;
					$tags_html .= "<ul class=\"list\">" ;
					/** fetch data from mysql **/
					$sql_query = "SELECT * FROM `remedios` ORDER BY `id`  DESC LIMIT 0 , 5" ;
					if($result = $mysql->query($sql_query)){
						while ($data = $result->fetch_array()){
							$tags_html .= "<li>" . htmlentities(stripslashes($data["titulo"])) . "</li>" ;
						}
					...
					$tags_html .= "</ul>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "<div class=\"panel-footer text-right\"><a href=\"?table=remedioss\" class=\"btn btn-sm btn-success\">".LANG_MORE."</a></div>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "</div>" ;
		break;
		// TODO: Categoriass
		case "categoriass":
					$tags_html .= "<h4>". @LANG_CATEGORIAS ."</h4>" ;
				switch($_GET["action"]){
					// TODO: ---- listing
					case "list":
					$tags_html .= "<ul class=\"nav nav-tabs\">" ;
					$tags_html .= "<li class=\"active\"><a href=\"?table=categoriass\">".@LANG_LIST."</a></li>" ;
					$tags_html .= "<li><a href=\"?table=categoriass&action=add\">".@LANG_ADD."</a></li>" ;
					$tags_html .= "</ul>" ;
					$tags_html .= "<br/>" ;
					...
					$table_html .= "<th>".@LANG_IMAGEM."</th>" ;
					$table_html .= "<th>".@LANG_DESCRICAO."</th>" ;
					...
					if($result = $mysql->query($sql_query)){
						while ($data = $result->fetch_array()){
							...
							$table_html .= "<td><img src=\"" . $data["imagem"] . "\" alt=\"#\" width=\"64\" height=\"64\" /></td>" ;
							$table_html .= "<td>" . htmlentities(stripslashes($data["descricao"])) . "</td>" ;
							$table_html .= "<td>" ;
							$table_html .= "<div class=\"btn-group\" >" ;
							$table_html .= "<a class=\"btn btn-sm btn-warning\" href=\"?table=categoriass&action=edit&id=". $data["id"]. "\"><span class=\"glyphicon glyphicon-pencil\"></span></a> " ;
							$table_html .= "<a onclick=\"return confirm('Are you sure you want to delete ID#". $data["id"]. "')\" class=\"btn btn-sm btn-danger\" href=\"?table=categoriass&action=delete&id=". $data["id"]. "\"><span class=\"glyphicon glyphicon-trash\"></span></a> " ;
							$table_html .= "</div>" ;
							$table_html .= "</td>" ;
							$table_html .= "</tr>" ;
							$table_html .= "\r\n" ;
			...
			case "add":
				// TODO: ---- add
				$tags_html .= "<ul class=\"nav nav-tabs\">" ;
				$tags_html .= "<li><a href=\"?table=categoriass&action=list\">".@LANG_LIST."</a></li>" ;
				$tags_html .= "<li class=\"active\"><a href=\"?table=categoriass&action=add\">".@LANG_ADD."</a></li>" ;
				$tags_html .= "</ul>" ;
				$tags_html .= "<br/>" ;
				...
				if(isset($_POST["add"])){
					...
					if(isset($_POST["imagem"])){
						$data_imagem = addslashes($_POST["imagem"]);
					}
				if(isset($_FILES["imagem-upload"]["name"])){
					if($_FILES["imagem-upload"]["name"]!=""){
						$ext = pathinfo($_FILES["imagem-upload"]["name"],PATHINFO_EXTENSION);
						$uploadfile =  "media/image/". sha1(time()).".".$ext;
						$uploadtemp =  $_FILES["imagem-upload"]["tmp_name"];
						...
						if(!is_dir(dirname(__FILE__)."/media/image/")) {
							...
							if (in_array(strtolower($ext),$config["image_allowed"])){
								if(preg_match("/image/",$mimetype["mime"])){
									...
									$data_imagem =  $full_url ."/". $uploadfile;
								}
							}
						}
					}
					if(isset($_POST["descricao"])){
						$data_descricao = addslashes($_POST["descricao"]);
					}
	
					/** prepare save to mysql **/
					$sql_query = "INSERT INTO `categorias` (`titulo`,`imagem`,`descricao`) VALUES ('$data_titulo','$data_imagem','$data_descricao')" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=categoriass&action=list");
				}
...
-->
<div class="form-group">
	<label for="imagem">' . @LANG_IMAGEM. '</label>
	<textarea id="imagem" class="form-control"  data-type="images" name="imagem" ></textarea>
	<p class="help-block">Using base64 image data <input data-target="#imagem" type="file" data-type="image-base64" /> or file upload <input name="imagem-upload" type="file" data-type="image-upload" /></p>
</div>
...
-->
<div class="form-group">
	<label for="descricao">' . @LANG_DESCRICAO. '</label>
	<input id="descricao" class="form-control" data-type="paragraph" type="text" name="descricao" placeholder="" />
	<p class="help-block"></p>
</div>
					<div class="form-group">
						<label for="add"></label>
		...
		case "edit":
			...
			if(isset($_GET["id"])){
				...
				if(isset($_FILES["imagem-upload"]["name"])){
					...
					/** update data to sql **/
					$sql_query = "UPDATE `categorias` SET `titulo` = '$data_titulo',`imagem` = '$data_imagem',`descricao` = '$data_descricao' WHERE `id`=$entry_id" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=categoriass&action=list");
			...
			/** fetch current data **/
			$sql_query = "SELECT * FROM `categorias`  WHERE `id`=$entry_id LIMIT 0,1" ;
			if($result = $mysql->query($sql_query)){
				while ($data = $result->fetch_array()){
					$rows[] = $data;
				}
			...
			if(isset($rows[0]["imagem"])){
				$data_imagem = stripslashes($rows[0]["imagem"]) ;
			}
			if(isset($rows[0]["descricao"])){
				$data_descricao = stripslashes($rows[0]["descricao"]) ;
			}
...
<div class="form-group">
	<label for="imagem">' . @LANG_IMAGEM. '</label>
	<textarea id="imagem" class="form-control"  data-type="images" name="imagem" >'.htmlentities($data_imagem).'</textarea>
	<p class="help-block">Using base64 image data <input data-target="#imagem" type="file" data-type="image-base64" /> or file upload <input name="imagem-upload" type="file" data-type="image-upload" /></p>
</div>
...
<div class="form-group">
	<label for="descricao">' . @LANG_DESCRICAO. '</label>
	<input id="descricao" class="form-control" data-type="paragraph" type="text" name="descricao" placeholder="" value="'.htmlentities($data_descricao).'" />
	<p class="help-block"></p>
</div>
<div class="form-group">
	...
	<input class="btn btn-primary" type="submit" name="edit" />
			...
			// TODO: ---- delete
				...
				if(isset($_GET["id"])){
					...
					$entry_id = (int)$_GET["id"];
					/** delete item in sql **/
					$sql_query = "DELETE FROM `categorias` WHERE `id`=$entry_id" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=categoriass&action=list");
		...
		// TODO: Remedioss
		case "remedioss":
					$tags_html .= "<h4>". @LANG_REMEDIOS ."</h4>" ;
				switch($_GET["action"]){
					// TODO: ---- listing
					case "list":
					$tags_html .= "<ul class=\"nav nav-tabs\">" ;
					$tags_html .= "<li class=\"active\"><a href=\"?table=remedioss\">".@LANG_LIST."</a></li>" ;
					$tags_html .= "<li><a href=\"?table=remedioss&action=add\">".@LANG_ADD."</a></li>" ;
					$tags_html .= "</ul>" ;
					$tags_html .= "<br/>" ;
					$tags_html .= "<div class=\"table-responsive\">" ;
					$table_html = "<table id=\"datatable\" class=\"table table-striped table-hover\">" ;
					...
					$table_html .= "<tr>" ;
					$table_html .= "<th>".@LANG_TITULO."</th>" ;
					$table_html .= "<th>".@LANG_TEXTO."</th>" ;
					$table_html .= "<th>".@LANG_IMAGEM."</th>" ;
					$table_html .= "<th>".@LANG_CATEGORIAS."</th>" ;
					$table_html .= "<th style=\"width:100px;\">Action</th>" ;
					$table_html .= "</tr>" ;
					$table_html .= "</thead>" ;
					$table_html .= "<tbody>" ;
					/** fetch data from mysql **/
					$sql_query = "SELECT * FROM `remedios`" ;
					if($result = $mysql->query($sql_query)){
						while ($data = $result->fetch_array()){
							$table_html .= "<tr>" ;
							$table_html .= "<td>" . htmlentities(stripslashes($data["titulo"])) . "</td>" ;
							$table_html .= "<td>" . htmlentities(stripslashes($data["texto"])) . "</td>" ;
							$table_html .= "<td><img src=\"" . $data["imagem"] . "\" alt=\"#\" width=\"64\" height=\"64\" /></td>" ;
							$table_html .= "<td>" . htmlentities(stripslashes($data["categorias"])) . "</td>" ;
							$table_html .= "<td>" ;
							$table_html .= "<div class=\"btn-group\" >" ;
							$table_html .= "<a class=\"btn btn-sm btn-warning\" href=\"?table=remedioss&action=edit&id=". $data["id"]. "\"><span class=\"glyphicon glyphicon-pencil\"></span></a> " ;
							$table_html .= "<a onclick=\"return confirm('Are you sure you want to delete ID#". $data["id"]. "')\" class=\"btn btn-sm btn-danger\" href=\"?table=remedioss&action=delete&id=". $data["id"]. "\"><span class=\"glyphicon glyphicon-trash\"></span></a> " ;
							$table_html .= "</div>" ;
							$table_html .= "</td>" ;
							$table_html .= "</tr>" ;
							$table_html .= "\r\n" ;
			...
			case "add":
				// TODO: ---- add
				$tags_html .= "<ul class=\"nav nav-tabs\">" ;
				$tags_html .= "<li><a href=\"?table=remedioss&action=list\">".@LANG_LIST."</a></li>" ;
				$tags_html .= "<li class=\"active\"><a href=\"?table=remedioss&action=add\">".@LANG_ADD."</a></li>" ;
				$tags_html .= "</ul>" ;
				$tags_html .= "<br/>" ;
				...
				if(isset($_POST["add"])){
					...
					if(isset($_POST["texto"])){
						$data_texto = addslashes($_POST["texto"]);
					}
					if(isset($_POST["imagem"])){
						$data_imagem = addslashes($_POST["imagem"]);
					}
				if(isset($_FILES["imagem-upload"]["name"])){
					...
					if(isset($_POST["categorias"])){
						$data_categorias = addslashes($_POST["categorias"]);
					}
	
					/** prepare save to mysql **/
					$sql_query = "INSERT INTO `remedios` (`titulo`,`texto`,`imagem`,`categorias`) VALUES ('$data_titulo','$data_texto','$data_imagem','$data_categorias')" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=remedioss&action=list");
				}
...
-->
<div class="form-group">
	<label for="texto">' . @LANG_TEXTO. '</label>
	<input id="texto" class="form-control" data-type="paragraph" type="text" name="texto" placeholder="" />
	<p class="help-block"></p>
</div>
...
<!--
// TODO: ----|-- form : CATEGORIAS
-->
<div class="form-group">
	<label for="categorias">' . @LANG_CATEGORIAS. '</label>
	<input id="categorias" class="form-control" data-type="text" type="text" name="categorias" placeholder="" />
	<p class="help-block"></p>
</div>
					<div class="form-group">
						<label for="add"></label>
		...
		case "edit":
			...
			if(isset($_GET["id"])){
				...
				if(isset($_FILES["imagem-upload"]["name"])){
					...
					/** update data to sql **/
					$sql_query = "UPDATE `remedios` SET `titulo` = '$data_titulo',`texto` = '$data_texto',`imagem` = '$data_imagem',`categorias` = '$data_categorias' WHERE `id`=$entry_id" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=remedioss&action=list");
			...
			/** fetch current data **/
			$sql_query = "SELECT * FROM `remedios`  WHERE `id`=$entry_id LIMIT 0,1" ;
			if($result = $mysql->query($sql_query)){
				while ($data = $result->fetch_array()){
					$rows[] = $data;
				}
			...
			if(isset($rows[0]["texto"])){
				$data_texto = stripslashes($rows[0]["texto"]) ;
			}
			if(isset($rows[0]["imagem"])){
				$data_imagem = stripslashes($rows[0]["imagem"]) ;
			}
			if(isset($rows[0]["categorias"])){
				$data_categorias = stripslashes($rows[0]["categorias"]) ;
			}
...
<div class="form-group">
	<label for="texto">' . @LANG_TEXTO. '</label>
	<input id="texto" class="form-control" data-type="paragraph" type="text" name="texto" placeholder="" value="'.htmlentities($data_texto).'" />
	<p class="help-block"></p>
</div>
...
<div class="form-group">
	<label for="categorias">' . @LANG_CATEGORIAS. '</label>
	<input id="categorias" class="form-control" data-type="text" type="text" name="categorias" placeholder="" value="'.htmlentities($data_categorias).'" />
	<p class="help-block"></p>
</div>
<div class="form-group">
	<label for="edit"></label>
	<input class="btn btn-primary" type="submit" name="edit" />
			...
			// TODO: ---- delete
				...
				if(isset($_GET["id"])){
					...
					$entry_id = (int)$_GET["id"];
					/** delete item in sql **/
					$sql_query = "DELETE FROM `remedios` WHERE `id`=$entry_id" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=remedioss&action=list");
		...
		$tags_html .= "<div class=\"container-fluid\">" ;
		$tags_html .= "<div class=\"row\">" ;
		$tags_html .= "<div class=\"col-md-12\"><p class=\"text-left navbar-text\">Steffan V.</p></div>" ;
		$tags_html .= "</div>" ;
